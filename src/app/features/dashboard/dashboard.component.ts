  ...
  template: `
    <div class="space-y-8 pb-10 animate-fade-in">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-6">
        <div>
          <span class="text-[10px] font-extrabold text-violet-400 uppercase tracking-widest block mb-1">REALTIME INTELLIGENCE</span>
          <h2 class="text-3xl font-black text-white tracking-tight">
            Executive Operations Dashboard
          </h2>
          <p class="text-xs text-slate-400 mt-1">Live inventory valuation, POS sales metrics, and stock movement streams</p>
        </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <!-- Module 1: Total Inventory Valuation (Violet) -->
        <div class="glass-card p-6 rounded-3xl space-y-4 shadow-glow-violet relative overflow-hidden border border-violet-500/20">
          <div class="flex items-center justify-between text-slate-400">
            <span class="text-xs font-mono uppercase tracking-wider text-violet-300 font-bold">Inventory Valuation</span>
            <div class="p-2.5 rounded-2xl bg-violet-600/20 text-violet-400 border border-violet-500/30">
        ...
        <!-- Module 2: Daily Sales Revenue (Emerald) -->
        <div class="glass-card p-6 rounded-3xl space-y-4 shadow-glow-emerald relative overflow-hidden border border-emerald-500/20">
          <div class="flex items-center justify-between text-slate-400">
            <span class="text-xs font-mono uppercase tracking-wider text-emerald-300 font-bold">Sales Revenue</span>
            <div class="p-2.5 rounded-2xl bg-emerald-600/20 text-emerald-400 border border-emerald-500/30">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Module 3: Procurement Spend (Cyan) -->
        <div class="glass-card p-6 rounded-3xl space-y-4 shadow-glow-cyan relative overflow-hidden border border-cyan-500/20">
          <div class="flex items-center justify-between text-slate-400">
            <span class="text-xs font-mono uppercase tracking-wider text-cyan-300 font-bold">Procurement Spend</span>
            <div class="p-2.5 rounded-2xl bg-cyan-600/20 text-cyan-400 border border-cyan-500/30">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Module 4: Low Stock Alerts (Amber) -->
        <div class="glass-card p-6 rounded-3xl space-y-4 shadow-glow-amber relative overflow-hidden border border-amber-500/20">
          <div class="flex items-center justify-between text-slate-400">
      ...
      <!-- MAIN BENTO GRAPH & ACTIVITY SECTION -->
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
          <div class="flex items-center justify-between border-b border-slate-800/80 pb-4">
            <div>
              <h3 class="text-lg font-extrabold text-slate-100">Category Breakdown & Inventory Balance</h3>
          ...
          <div class="space-y-5">
            @for (cat of dashboardFacade.categoryBreakdown$ | async; track cat.categoryName) {
              <div class="space-y-2">
                ...
                <div class="w-full h-3.5 bg-slate-900 rounded-full overflow-hidden p-0.5 border border-slate-800 shadow-inner">
                  <div 
        ...
        <div class="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6 flex flex-col justify-between">
          <div>
            ...
            <div class="mt-4 space-y-3 max-h-80 overflow-y-auto pr-1">
              @for (item of stockFacade.movements$ | async; track item.id) {
                <div class="p-3.5 rounded-2xl bg-slate-900/90 border border-slate-800 flex items-center justify-between text-xs hover:border-slate-700 transition-all">
                  <div class="flex items-center space-x-3">
                    <div 
          ...
          <a 
            routerLink="/stock"
            class="w-full py-3 rounded-2xl bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 text-center text-xs font-bold block transition-all"
          >
